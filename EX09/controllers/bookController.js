...
const CreateBook=async(req,res)=>{
    const{image,name,code,author, price,language,type,popular,cover}=req.body
    console.log(req.user)
    if (!name || !code || !price){
        return res.status(400).json({message:'name,code and price are required'})
    }
    const book=await Book.create({image,name,code,author, price,language,type,popular,cover})
    if (book){
